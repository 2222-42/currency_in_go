...
[この区別は筆者の見解で独特なもののような感じがするらしい。後の章を読んで、また、ほかの人の意見を聞けば、また新たな理解が得られるかもしれない]
...
ホーアは入力と出力がプログラミング（特に平行なコード）においてのプリミティブとして見落とされていると提案[当時、ほかにも並行性に関する研究はあった(ラムダ計算の研究がある程度整って並行性にみんな着目していっていた)から、ホーアのCSPは優れてはいるが、生き残っただけなのかもしれない]。並行処理への感心が寄せられていなかった状況を正すべく論文を作成。[訳注にもある通り、ホーアはそんな意図はない]
...
しかし、CSPの原理を言語の中核として具現化したGoが公開されるまでは、これらのプリミティブを言語自体でサポートしていたプログラミング言語ではほとんどなかった。[ライブラリとしてはサポートされているから筆者の意図が強すぎやしないか？]
...
結論: 簡潔さを求め、チャネルをできる限り使い、ゴルーチンを湯水のように使いましょう

GoはCSPに設計の大部分で影響を受けているが、メモリアクセス同期とそれに応じたプリミティブを通した並行処理のコードをかけるようにしています。syncの中の構造体とメソッド、また他のパッケージはロックの実行、リソースプールの作成、ゴルーチンの割り込みなどが可能。

Goでの並列処理の書き方はCSP唯一ではない。メモリアクセス同期のプリミティブは公開されている。高水準のものが推奨されているのは確かであり、モットーは「通信によってメモリを共有し、メモリの共有によって通信してはいけない」だが、最も適切に表現できて、かつあるいはまたは、最も簡潔に書けるなら、syncパッケージでの伝統的なロック機能かチャネルかどちらでもよい。

決定木を見れば、基準がある程度明確になる。ゴルーチンはばんばん使え。ハードウェアがサポートする限界までゴルーチンを起動することになるより前にプログラムの再設計の方が先になるだろうし。

- 「データの所有権を移動しようとしているか」(しているなら、チャネルを使う。): 所有権の移動とは、何かしら結果を生成するコードがあり、その結果を別のコードに共有したい場合のこと。並行プログラムを安全にする方法の1つは、1度に1つの並行処理のコンテキストのみがデータの所有権を持つようにすることであり、チャネルを使えば安全にするという構想を伝えることができる。
- 「構造体の内部の状態を保護しようとしているか」(しているなら、プリミティブを使う): メモリアクセス同期を使うことで、クリティカルセクションをロックする実装の複雑な詳細を呼び出し元から隠せるから。型を超えてロックを公開せず、内向きに、レキシカルなスコープ(関数のスコープ内で限定的に使用できる擬似的な静的スコープ)内に制限しよう。
- 「複数のロジックを協調させようとしているか」(しているならチャネルを、していないなら、プリミティブを): チャネルは構成可能であるから。Goのselect文をチャネル内で使えば、複雑さを管理するのがとても簡単になるし、チャネルのキューとしての能力と安全に取りまわせる。
- 「パフォーマンスクリティカルセクションか」(そうだったらプリミティブを): メモリアクセス同期を使っていれば、負荷がかかったときでもクリティカルセクションがよりよく動作するだろう。ただし、こうした考慮をする前に、もしクリティカルセクションがあるなら再設計したほうがいい。
