...
チャネルはHoareのCSP由来。チャネルはメモリに対するアクセスを同期するのに使える一方で、ゴルーチン間の通信に使うのが最適。

### チャネルとはなにか

チャネルは "Stream"感がある(変数として後置語に使うのは"Ch"の方が一般的)。

プログラム同士が何をしているかｗ知らず、チャネルが存在しているメモリ中の同じ場所を参照している。

### チャネルの使い方

値をchan型の変数に渡し、プログラムのどこかほかの場所でその値をチャネルから読み込む。

#### チャネルの作り方

1. チャネルを宣言( `chan interface{}` )。
2. ビルトイン関数`make`関数を使ってチャネルを初期化。
...
一方向チャネルを初期化することはあまり見かけないが、関数の引数や戻り値としてはよく使われる。便利。Goは双方向チャネルを必要に応じて暗黙的に一方向チャネルに変換している。

チャネルは型であり、どのような型のデータでも出し入れできる( `interface{}`でも `int`でも `string`でも )。

Goの型システムのおかげで並行処理のプリミティブであっても型安全に使える。読み込み専用チャネルにデータを書き込もうとしたり、書き込み専用チャネルから読み込もうとしたりする場合、コンパイルエラーが発生するから。

Goのチャネルは(書き込みの場合は空きが出るまで待機し、読み込みの場合は少なくとも要素が1つまで待機し)ブロックする；だから、メインのゴルーチンが終わる前にちゃんと実行される；ブロックするためプログラムを正しく書かないとデッドロックが引き起こされる　→　デッドロックの予防方法は次の章で説明する。

チャネルからの読み込み・受信は、オプションとして2つの値を返せる。
1. 1つ目の戻り値はチャネルの要素の値、
2. 2つ目の戻り値はプロセス内のどこかで書き込みがあったことで読み込み演算子が読み込みできたかどうか、あるいは、閉じたチャネルから生成されたデフォルト値のいずれか

#### チャネルの閉じ方

閉じたチャネル・チャネルを閉じることについて；これ以上チャネルから値が送られてこないことを示す方法(データ型で解決するのは治安が悪い)であり、普遍的な見張りのようなもの

チャネルの閉じ方: (`close(nameChannel)`)

閉じたチャネルでも読み込める理由：1つの上流での書き込みに対して、複数の下流で読み込みができるようにするため(cf: chapter 4)

閉じたチャネルによって使えるいくつかの新しいパターン:

1. チャネルをループで処理する；rangeキーワードはチャネルを引数にとり、チャネルが閉じたときに自動的にループを終了する。;閉じたチャネルの処理については`range`キーワードが代わりに判断してくれる。
2. チャネルを閉じることで、複数のゴルーチンに同時にシグナルを送信できる。(`sync.Cond`でもよいが、チャネルの方が組み合わせやすい)

#### チャネルのバッファ

バッファ付きチャネル；初期化の際にキャパシティが与えられたチャネルのこと；読み込みが行われなくてもバッファの分`n`だけゴルーチンは書き込み可能

初期化するゴルーチンがそのチャネルにバッファがあるかどうかを管理している　→　バッファ付きチャネルの性質への影響

バッファなしチャネルは、バッファ付きチャネルでキャパシティが0のもののこと。`make`の第二引数を指定しなかったり0に指定したりすればそれ。

キャパシティはバッファサイズのこと　→　バッファ付きチャネルはあるいみ並行プロセスが通信するためのインメモリのFIFOキュー

キャパシティ4のバッファ付きチャネルの例:

1. `make(chan rune, 4)`([,,,])
2. `c <- 'A'` ([A, , ,])　
3. `c <- 'B', c <- 'C', c <- 'D'` ([A, B, C, D])
4. c <- 'E'、ここでゴルーチンはブロックされる。このブロックは、このチャネルに対して別のゴルーチンが読み込みをして空きができるまで。
5. <-c、ここで、ブロックされていた書き込みが解放されて、Eがバッファの末尾に追加される。([B, C, D, E])

バッファ付きチャネルが空で、読み込みがブロックされていた場合は、バッファはバイパスされ、値は送信元から受信先へと直接渡される。
...
- read nil channel -> block -> deadlock -> panic
- write nil channel -> block -> deadlock -> panic
...
|---|---|---|
| read | nil | block |
|  | Open & NonEmpty | Get value |
|  | Open & Empty | block |
|  | Close | Get <default value, false> |
|  | WriteOnly | Compile Error |
| write | nil | block |
|  | Open & NotFull | Write value |
|  | Open & Full | block |
|  | Close | panic |
|  | ReadOnly | Compile Error |
| close | nil | panic |
|  | Open & NotEmpty | Close. But readable. After became empty, read default value. |
|  | Open & Empty | Close. Read default Value |
|  | Closed | panic |
|  | ReceiveOnly | Compile Error |
...
#### チャネルを使う際のリマーク（プログラムの構成方法）

上記のチャネルでの問題が起きる組み合わせを観れば、堅牢で安定したシステムを構築するために、異なる型のチャネルをどう組み合わせればよいかを示す

1. チャネルの所有権を割り振る(チャネルを初期化し、書き込むゴルーチン、閉じるゴルーチンがいずれかをはっきりさせる)
    1. チャネルを初期化する
    2. 書き込みを行うか、ほかのゴルーチンに所有権を渡す(nil channel にかきこむことはなくなる、コンパイル時の型チェックでチャネルに対する不適切な書き込みを防げる)
    3. チャネルを閉じる(nil channelを閉じることはなくなるし、二度閉じることはない)
        1. 所有者の責任として確実に閉じる。
    4. 上の3つの手順をカプセル化して読み込みチャネルを経由して公開
        1. 他のゴルーチンに渡すよりも先にチャネルを作成することで、外の関数によってカプセル化される。
2. 読み込みの際はブロックしうるという事実を扱うべきである
    - チャネルがいつ閉じられたかを把握する。(2つ目の戻り値を確認する)
    - いかなる理由でもブロックする操作は責任をもって行う。

チャネルの所有権のスコープは小さく。やっていることが自明になる。明瞭さが重要。　→　デッドロックやパニックの原因は大体ここ。
...
チャネルがゴルーチンを結びつける糊、select文はチャネルをまとめる糊で、より大きな抽象化をするためにチャネルを組み合わせられる。

select 文は強力。局所的(単一の関数や型でのチャネルの紐づけ)にも大局的(システム内のいくつかのコンポーネントの紐づけ)にも紐づけられる。(大規模開発での場合の組み合わせについては5章で説明)

select文とは何か、どう使うか、どのように動作するか

case文と構造だけ似ている。
- 上から順には評価されないし、
- 1つも条件に該当されなかったら自動的に実行されない; ブロックされる。
- 読み込みや書き込みのチャネルはすべて同時に扱われ、どれが用意できたか(書き込まれたか、閉じられたか、キャパシティがいっぱいか)を確認する

疑問：
1. 複数のチャネルが読み込んでいる場合　→　ならして実行される
2. どのチャネルも準備完了しなかった場合　→　デッドロックされるから、`time.After`関数を用いてタイムアウトさせる方法がある(4章でこのパターンについて改めて紹介し、より堅牢な解決方法を紹介)
3. いずれのちゃねるも準備完了していない場合に何かしたい場合 -> `default`節を使おう。

疑問1について：Goのランタイムはcase文全体に対して疑似乱数による一様選択をする。なぜならGoのランタイムが実現したい最善の状態は、select文にまとめられた理由がわからないのだから、それぞれの条件をならして実行すること。

疑問3について：パターンとしてfor文の中にselect文を入れる。defaultに何もかかないことで、select文の次のfor文ないのプログラムに移ることができる。

空select文`select {}`: 永遠にブロックする。

## 3.5 GOMAXPROCSレバー

「ワークキュー」と呼ばれるOSスレッドの数を制御。

Goは1.5以降のヴァージョンでは自動的にホストマシンの論理CPUの数に設定されるようになった。

これを使うのはテストのところ。例えば、論理CPUコア数以上に増やすことで、競合状態をより多く発生させることができて、問題の発見と修正ができる。

`GOMAXPROCS`を調整することで、プログラムは生身のハードウェアに近づくが、それは抽象化と長期的な性能の安定性を犠牲にするもの。

## 3.6 まとめ

この章で扱ったこと:

- Goが提供している自由に使える並行処理の基本的なプリミティブ →　着実に性能がよく可読性が高く論理的にも正しいプログラムが書けるようになっている
- syncパッケージのメモリアクセス同期のプリミティブを使うタイミングがいつか
- チャネルとselect文を使って「通信によってメモリを共有する」のがより適切なタイミングがいつか

残っていること：

- これらのプリミティブをスケールしつつ理解りやすい構造に組み合わせていく方法とパターン

### 補足: Atomic

`sync/atomic`パッケージ: アトミックな操作を提供しているパッケージ
...
これを使えば、アトミックにするためにミューテックスを使わずに、簡潔に記述できる。

`atomic.Value`という構造体もあり、LoadとStoreというメソッドがあり、これで任意の方の読み込みと書き込みをアトミックにできる
