...
ランタイムの動作に関する知識は役に立つし、ランタイムのおかげで並行性のモデル化での議論はなりたつ。

裏側はすごい叡智の量で、性能がより高いものにする素晴らしい仕事をし、詳細の抽象化をし開発者がさわる部分にはとても単純な表面だけを見せている

## 6.1 ワークスティーリング

GoがゴルーチンをOSスレッド上へのマルチプレキシングをするが、そこで使っているアルゴリズムはワークスティーリング(仕事を奪う)戦略
...
規則に従ったコールスタックとワークデックの表でタスクスティーリングの流れを見てみよう：
...
タスクと継続とで、ワークスティーリングアルゴリズムにおおいて、何が違うのか？

合流地点について検討しよう：
- 完了条件を満たしていない合流地点に到達(停止した合流)すると、実行している処理を停止して、タスクを盗む
- タスクスティーリング(別名「チャイルドスティーリング」)も継続スティーリングも停止した合流があるが、停止が発生する頻度に重要な違いがある。
- なお、原則から考えて、ゴルーチンをスケジュールするときは即時にそれを処理するのが理にかなっている

スレッドの性質について検討しよう：
- 継続をデックの最後尾に入れる　→　その継続を盗まれる確率は減る　→　呼び戻しやすい　→　停止をさけられる（分岐したタスクは関数呼び出しにとても良く似たものになる）
- つまり、スレッドはゴルーチンの実行にジャンプして、その処理が完了してから継続に戻る
...
call `fib(4)` and 個の呼び出しからの継続がT1のデックの最後尾に追加:

| T1 call stack | T1 work deque | T2 call stack | T2 work deque |
|---|---|---|---|
| fib(4) | mainの継続 |  |  |

この時、mainの継続を盗むのはアイドルなもの、つまりT2.

| T1 call stack | T1 work deque | T2 call stack | T2 work deque |
|---|---|---|---|
| fib(4) |  | mainの継続 |  |

継続なので、fib(3)とfib(2)を一度に追加するのではなく、fib(4)の呼び出しはfib(3)をその後スケジュールする

| T1 call stack | T1 work deque | T2 call stack | T2 work deque |
|---|---|---|---|
| fib(3) | fib(4)の継続 | mainの継続 |  |

T2はmainの継続を実行しようとして、未達の合流地点に到達
→　よってT1から仕事を盗む。

| T1 call stack | T1 work deque | T2 call stack | T2 work deque |
|---|---|---|---|
| fib(3) |  | mainの継続(未達の合流地点) |  |
|  |  | fib(4)の継続 |  |

継続なので、fib(2)とfib(1)を一度に追加するのではなく、fib(3)の呼び出しはfib(2)をその後スケジュールする

| T1 call stack | T1 work deque | T2 call stack | T2 work deque |
|---|---|---|---|
| fib(2) | fib(3)の継続 | mainの継続(未達の合流地点) |  |
|  |  | fib(4)の継続 |  |

T2のfib(4)の継続の実行は、fib(3)はやっていたので、fib(2)をその後スケジュールする

| T1 call stack | T1 work deque | T2 call stack | T2 work deque |
|---|---|---|---|
| fib(2) | fib(3)の継続 | mainの継続(未達の合流地点) | fib(4)の継続 |
|  |  | fib(2) |  |
...
| T1 call stack | T1 work deque | T2 call stack | T2 work deque |
|---|---|---|---|
| (1を返す) | fib(3)の継続 | mainの継続(未達の合流地点) | fib(4)の継続 |
|  |  | fib(2) |  |
...
T1はfib(3)の継続を盗み、fib(1)の実行を開始(コールチェインがなめらかな感じがするね。`fib(3) -> fib(2) -> fib(1)`と直列)

| T1 call stack | T1 work deque | T2 call stack | T2 work deque |
|---|---|---|---|
| fib(1)(fib(3)の継続より) |  | mainの継続(未達の合流地点) | fib(4)の継続 |
|  |  |  |  |

T2はfib(4)の継続を盗むが、fib(3)の呼び出しはT1で実行されているので、未達の合流地点で、アイドルとなる

| T1 call stack | T1 work deque | T2 call stack | T2 work deque |
|---|---|---|---|
| fib(1) |  | mainの継続(未達の合流地点) |  |
|  |  | fib(4)(未達の合流地点；継続の最後fib(4)にいる) |  |

| T1 call stack | T1 work deque | T2 call stack | T2 work deque |
|---|---|---|---|
| (2(1+1)を返す)(継続の最後fib(3)のにいる) |  | mainの継続(未達の合流地点) |  |
|  |  | fib(4)(未達の合流地点；継続の最後fib(4)にいる) |  |
...
比較すると
- ステップは少なくなり、
- ストールした合流地点は、全てアイドルスレッド上で行われ、かつ少なくなり、
- コールスタックのサイズも減る

(「ストールした合流地点」ってなんだ？ stealの過去形のstoleか？ 未達の合流地点への合流のことか？　確かに合流地点をどんどん盗んで行っている)
...
(継続スティールの場合、ストールしないって書いているけれど、ストールしたって筆者自身が書いているから謎。(ビジースレッド上ではやらなかったくらいの意味？))

なんですべてのワークスティーリングアルゴリズムは継続スティーリングを実装していないのか
→　継続スティーリングは通常コンパイラのサポートが必要になるから。
→　Goは継続スティーリングを採用している
...
入出力（チャネルの？）もシステムコールも関数呼び出しもしていないゴルーチンは例外で問題として残っている
