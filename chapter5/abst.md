...
（ハートビートで十分でいい、というのは、A2を起動しなくて良くなるから。）
...
長時間起動しているゴルーチンが健全な状態で起動し続ける助けになるパターンの学習

ゴルーチンが外部の助けなしには回復できない悪い状態にとどまってしまうことがありうる。

- 学ぶこと：「回復」する仕組みを作ること
- 回復とは：ゴルーチンが不健全な状態になったら再起動する流れ

ハートビートパターンを使おう

使うハートビートの種類は何を監視するかに依存する
- ライブロックなら、ゴルーチンが起動しているかだけでなく、実際に処理をしているかも確認する

→　本節では単純のため、死活だけ確認する。

- 管理人(steward): ゴルーチンの健全性を監視するロジック：中庭が不健全になったら再起動する責任を持つ、つまりゴルーチンを起動する関数への参照が必要
- 中庭(ward): 管理人が監視するゴルーチン
...
複数起動する可能性があるから、bridgeチャネルを利用して、単一の妨げられないチャネルをdoWorkの消費者に渡す手助けをする

リマーク：
- 中庭を開発するとき、システムが重複した値に対して影響を受けやすい場合もあるので、再起動の際に毎回初めから起動されていることは念頭に置こう。

一定数以上の失敗が起きた後に起動する管理人の実装
